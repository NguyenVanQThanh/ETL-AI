...
LOG="${HOME}/.cache/claude-code-etl-ai/orchestrator-gate.log"
...
# core dir alternation = src (vd src|lib). Rỗng → skip (no core).
ALT='src'
if [ -n "$ALT" ]; then
  P=(
...
🚦 orchestrator-gate: MAIN agent (orchestrator — mọi model) KHÔNG ghi vào source core qua Bash.
   ...
   (src/) PHẢI route qua delegate subagent (Task tool):
